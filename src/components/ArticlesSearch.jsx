...
function ArticlesSearch({ setArticlesArray, setArticlesLength, limit, p, isLoading, setIsLoading, error, setError }) {
  ...
  useEffect(() => {
    setIsLoading(true)
    axios
      .get(`https://northcoders-news-api-phe8.onrender.com/api/articles?topic=${topic}&sort_by=${sortBy}&order=${order}&limit=${limit}&p=${p}`)
      .then(({data}) => {
        // console.log(data);
        setIsLoading(false)
        setError(null)
  ...
  return (
    <search>
      <form action="">
        ...
        <select name="Order" id="Order" onChange={orderOnChange} value={order}>
